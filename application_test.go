...
// MockReloadOrchestrator provides a mock implementation for testing
type MockReloadOrchestrator struct {
	mock.Mock
}

func (m *MockReloadOrchestrator) RequestReload(ctx context.Context, sections ...string) error {
	args := m.Called(ctx, sections)
	return args.Error(0)
}

// MockHealthAggregator provides a mock implementation for testing
type MockHealthAggregator struct {
	mock.Mock
}

func (m *MockHealthAggregator) Collect(ctx context.Context) (AggregatedHealth, error) {
	args := m.Called(ctx)
	return args.Get(0).(AggregatedHealth), args.Error(1)
}
...
func TestRequestReloadServiceIntegration(t *testing.T) {
	t.Run("should use registered ReloadOrchestrator service", func(t *testing.T) {
		// Setup application with mock logger
		mockLogger := &MockLogger{}
		app := NewStdApplication(nil, mockLogger).(*StdApplication)

		// Set up expectations for logger debug messages during service registration
		mockLogger.On("Debug", "Registered service", mock.Anything).Return()

		// Register a mock ReloadOrchestrator
		mockOrchestrator := &MockReloadOrchestrator{}
		err := app.RegisterService("reloadOrchestrator", mockOrchestrator)
		if err != nil {
			t.Fatalf("Failed to register mock orchestrator: %v", err)
		}

		// Set up expectation for the orchestrator
		mockOrchestrator.On("RequestReload", mock.Anything, []string{"section1", "section2"}).Return(nil)

		// Set up expectation for logger
		mockLogger.On("Info", "RequestReload called", mock.Anything).Return()

		// Call RequestReload
		err = app.RequestReload("section1", "section2")

		// Should succeed and delegate to orchestrator
		if err != nil {
			t.Errorf("RequestReload should succeed with registered orchestrator, got error: %v", err)
		}
	...
	t.Run("should return helpful error when ReloadOrchestrator not registered", func(t *testing.T) {
		// Setup application without ReloadOrchestrator service
		mockLogger := &MockLogger{}
		app := NewStdApplication(nil, mockLogger).(*StdApplication)
		...
		if err == nil {
			t.Error("RequestReload should return error when ReloadOrchestrator not registered")
		}

		expectedError := "dynamic reload not available - use WithDynamicReload() option when creating application"
	...
	t.Run("should handle orchestrator service lookup error gracefully", func(t *testing.T) {
		// Setup application
		mockLogger := &MockLogger{}
		app := NewStdApplication(nil, mockLogger).(*StdApplication)
		...
		// Register something that's not a ReloadOrchestrator
		err := app.RegisterService("reloadOrchestrator", "not an orchestrator")
		if err != nil {
			t.Fatalf("Failed to register fake service: %v", err)
		}

		// Set up expectation for logger
		mockLogger.On("Info", "RequestReload called", mock.Anything).Return()

		// Call RequestReload - this will fail when trying to cast to ReloadOrchestrator
		err = app.RequestReload("test")

		// Should return helpful error about dynamic reload not being available
		if err == nil {
			t.Error("RequestReload should return error when service is wrong type")
		}
...
func TestHealthAccessorMethodIntegration(t *testing.T) {
	t.Run("should return registered HealthAggregator service", func(t *testing.T) {
		// Setup application with mock logger
		mockLogger := &MockLogger{}
		app := NewStdApplication(nil, mockLogger)

		// Set up expectations for logger debug messages during service registration
		mockLogger.On("Debug", "Registered service", mock.Anything).Return()

		// Register a mock HealthAggregator
		mockAggregator := &MockHealthAggregator{}
		err := app.RegisterService("healthAggregator", mockAggregator)
		if err != nil {
			t.Fatalf("Failed to register mock aggregator: %v", err)
		}

		// Call Health accessor method - this should work with registered aggregator
		healthAgg, err := app.Health()

		// Should succeed and return the aggregator
		if err != nil {
			t.Errorf("Health() should succeed with registered aggregator, got error: %v", err)
		}
		if healthAgg == nil {
			t.Error("Health() should return non-nil aggregator")
		}
		if healthAgg != mockAggregator {
			t.Error("Health() should return the registered aggregator")
		}

		mockLogger.AssertExpectations(t)
	})

	t.Run("should return helpful error when HealthAggregator not registered", func(t *testing.T) {
		// Setup application without HealthAggregator service
		mockLogger := &MockLogger{}
		app := NewStdApplication(nil, mockLogger)

		// Call Health accessor method
		_, err := app.Health()

		// Should return helpful error
		if err == nil {
			t.Error("Health() should return error when HealthAggregator not registered")
		}

		expectedError := "health aggregator not available"
	...
	t.Run("should handle aggregator service type error gracefully", func(t *testing.T) {
		// Setup application
		mockLogger := &MockLogger{}
		app := NewStdApplication(nil, mockLogger)
		...
		// Register something that's not a HealthAggregator
		err := app.RegisterService("healthAggregator", "not an aggregator")
		if err != nil {
			t.Fatalf("Failed to register fake service: %v", err)
		}

		// Call Health accessor method - this will fail when trying to cast to HealthAggregator
		_, err = app.Health()

		// Should return helpful error about aggregator not being available
		if err == nil {
			t.Error("Health() should return error when service is wrong type")
		}

		expectedError := "invalid health aggregator service"
		if !strings.Contains(err.Error(), expectedError) {
			t.Errorf("Expected error message to contain '%s', got '%s'", expectedError, err.Error())
		}
...
func TestConfigurationIntegrationTasks(t *testing.T) {
	t.Run("T044-T046 integration: Dynamic field parsing, RequestReload, and Health methods", func(t *testing.T) {
		// Test that all three implemented features work together

		// Test T044: Dynamic field tag parsing
		...
		if err != nil {
			t.Fatalf("Dynamic field parsing failed: %v", err)
		}

		if len(dynamicFields) != 1 || dynamicFields[0] != "DynamicField1" {
			t.Errorf("Expected 1 dynamic field 'DynamicField1', got %v", dynamicFields)
		}

		// Test T045 & T046: RequestReload and Health integration
		mockLogger := &MockLogger{}
		app := NewStdApplication(nil, mockLogger)
		...
		err = app.RegisterService("reloadOrchestrator", mockOrchestrator)
		if err != nil {
			t.Fatalf("Failed to register orchestrator: %v", err)
		}
		...
		// Test T045: RequestReload method
		mockLogger.On("Info", "RequestReload called", mock.Anything).Return()
		mockOrchestrator.On("RequestReload", mock.Anything, []string{"test"}).Return(nil)

		err = app.RequestReload("test")
		if err != nil {
			t.Errorf("RequestReload should work with registered orchestrator: %v", err)
		}

		// Test T046: Health accessor method
		healthAgg, err := app.Health()
		if err != nil {
			t.Errorf("Health() should work with registered aggregator: %v", err)
		}
		if healthAgg != mockAggregator {
			t.Error("Health() should return the registered aggregator")
		}
		...
		t.Log("All configuration integration tasks (T044-T046) working correctly!")
	})
}

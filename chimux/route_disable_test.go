...
func (c *testEventCollector) OnEvent(ctx context.Context, e cloudevents.Event) error {
...
func TestDisableRoute_Basic(t *testing.T) {
	...
	// Register route
	module.Get("/disable-me", func(w http.ResponseWriter, r *http.Request) {
	...
	found := false
	for _, et := range collector.events {
		if et == EventTypeRouteRemoved {
			found = true
			break
		}
	}

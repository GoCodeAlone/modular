...
func TestMemorySubscriptionLifecycle(t *testing.T) {
	...
	// First unsubscribe
	if err := mem.Unsubscribe(ctx, sub); err != nil {
	...
	if err := mem.Unsubscribe(ctx, sub); err != nil {
		t.Fatalf("second unsubscribe should be no-op, got error: %v", err)
	}

	// Direct double cancel path also returns nil.
	if err := sub.Cancel(); err != nil {
		t.Fatalf("second direct cancel: %v", err)
	}

	// Publish events to confirm no delivery after unsubscribe.
	if err := mem.Publish(ctx, Event{Topic: "lifecycle.topic"}); err != nil {
		t.Fatalf("publish: %v", err)
	}
	newDelivered, newDropped := mem.Stats()
	if newDelivered != delivered || newDropped != dropped {
		t.Fatalf("expected stats unchanged after publishing to removed subscription: got %d/%d -> %d/%d", delivered, dropped, newDelivered, newDropped)
	}
}

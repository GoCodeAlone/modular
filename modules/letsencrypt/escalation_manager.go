...
func (c *EscalationConfig) setDefaults() {
    if c.FailureThreshold == 0 { c.FailureThreshold = 3 }
    if c.Window == 0 { c.Window = 5 * time.Minute }
    if c.ExpiringSoonDays == 0 { c.ExpiringSoonDays = 7 }
    if c.RateLimitSubstring == "" { c.RateLimitSubstring = "rateLimited" }
    if c.NotificationCooldown == 0 { c.NotificationCooldown = 15 * time.Minute }
}
...
func WithNow(fn func() time.Time) func(*EscalationManager) { return func(m *EscalationManager) { m.now = fn } }
...
func (m *EscalationManager) CheckExpiration(ctx context.Context, domain string, certInfo *CertificateInfo) (*CertificateRenewalEscalatedEvent, error) {
    if certInfo == nil { return nil, fmt.Errorf("nil certInfo") }
    if !certInfo.IsExpiringSoon(m.cfg.ExpiringSoonDays) { return nil, nil }
    m.mu.Lock(); defer m.mu.Unlock()
...
func (m *EscalationManager) Acknowledge(domain string) {
    m.mu.Lock(); defer m.mu.Unlock()
    if st, ok := m.domains[domain]; ok { st.acknowledged = true }
}
...
func (m *EscalationManager) Clear(ctx context.Context, domain string) {
    ...
    st.active = false
    st.failures = 0
    if m.eventEmitter != nil { _ = m.eventEmitter(ctx, rec) }
}
...
func (m *EscalationManager) Stats() EscalationStats {
    ...
    for k,v := range m.stats.Reasons { reasons[k]=v }
...
func (m *EscalationManager) escalateLocked(ctx context.Context, domain string, et EscalationType, errMsg string) (*CertificateRenewalEscalatedEvent, error) {
    ...
    m.notify(ctx, evt)
    if m.eventEmitter != nil { _ = m.eventEmitter(ctx, evt) }
    return evt, nil
}

func (m *EscalationManager) notify(ctx context.Context, evt *CertificateRenewalEscalatedEvent) {
    for _, ch := range m.channels { _ = ch.Notify(ctx, evt) }
}
...
func (m *EscalationManager) maybeRenotifyLocked(ctx context.Context, domain string, st *escalationState, errMsg string) *CertificateRenewalEscalatedEvent {
    if !st.active || st.acknowledged { return nil }
    now := m.now()
    if now.Sub(st.lastNotification) < m.cfg.NotificationCooldown { return nil }
    st.lastNotification = now
...
func (e *CertificateRenewalEscalationRecoveredEvent) EventType() string  { return "certificate.renewal.escalation.recovered" }
func (e *CertificateRenewalEscalationRecoveredEvent) EventSource() string { return "modular.letsencrypt" }
